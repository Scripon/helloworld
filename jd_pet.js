...
[task_local]
#东东萌宠
15 7-21/7 * * * jd_pet.js, tag=东东萌宠, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdmc.png, enabled=true

=================================Loon===================================
[Script]
cron "15 7-21/7 * * *" script-path=jd_pet.js,tag=东东萌宠

===================================Surge================================
东东萌宠 = type=cron,cronexp="15 7-21/7 * * *",wake-system=1,timeout=3600,script-path=jd_pet.js

====================================小火箭=============================
东东萌宠 = type=cron,script-path=jd_pet.js, cronexpr="15 7-21/7 * * *", timeout=3600, enable=true
...
function runTimes(){
  return new Promise((resolve, reject) => {
    $.get({
        url: `${$.isNode() ? require('./USER_AGENTS').hwApi : 'https://api.jdsharecode.xyz/api/'}runTimes?activityId=pet&sharecode=${$.petInfo.shareCode}`
      }, (err, resp, data) => {
        if (err) {
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `${$.isNode() ? require('./USER_AGENTS').hwApi : 'https://api.jdsharecode.xyz/api/'}pet/${randomCount}`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
