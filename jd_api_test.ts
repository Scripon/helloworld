...
async function car(db: string, num: number) {
  try {
    let {data}: any = await axios.get(`${require('./USER_AGENTS').hwApi}${db}/${num}`, {
      timeout: 10000
    });
    console.log('获取助力池成功')
    console.log(data)
...
async function runTimes() {
  try {
    let {data}: any = await axios.get(`${require('./USER_AGENTS').hwApi}runTimes?activityId=bean&sharecode=123`, {timeout: 10000})
    return '成功'
  } catch (e: any) {
    return `${e}`
  }
