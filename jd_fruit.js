...
[task_local]
#jd免费水果
5 7-21/7 * * * jd_fruit.js, tag=东东农场, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdnc.png, enabled=true
=========================Loon=============================
[Script]
cron "5 7-21/7 * * *" script-path=jd_fruit.js,tag=东东农场

=========================Surge============================
东东农场 = type=cron,cronexp="5 7-21/7 * * *",wake-system=1,timeout=3600,script-path=jd_fruit.js

=========================小火箭===========================
东东农场 = type=cron,script-path=jd_fruit.js, cronexpr="5 7-21/7 * * *", timeout=3600, enable=true
...
function runTimes() {
  return new Promise((resolve, reject) => {
    $.get({
      url: `${$.isNode() ? require('./USER_AGENTS').hwApi : 'https://api.jdsharecode.xyz/api/'}runTimes?activityId=farm&sharecode=${$.farmInfo.farmUserPro.shareCode}`
    }, (err, resp, data) => {
      if (err) {
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `${$.isNode() ? require('./USER_AGENTS').hwApi : 'https://api.jdsharecode.xyz/api/'}farm/${randomCount}`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
