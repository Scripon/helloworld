...
[task_local]
#东东工厂
40 8-23/4 * * * jd_jdfactory.js, tag=东东工厂, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jd_factory.png, enabled=true

================Loon==============
[Script]
cron "40 8-23/4 * * *" script-path=jd_jdfactory.js,tag=东东工厂

===============Surge=================
东东工厂 = type=cron,cronexp="40 8-23/4 * * *",wake-system=1,timeout=3600,script-path=jd_jdfactory.js

============小火箭=========
东东工厂 = type=cron,script-path=jd_jdfactory.js, cronexpr="40 8-23/4 * * *", timeout=3600, enable=true
 */
const $ = new Env('东东工厂');
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `${$.isNode() ? require('./USER_AGENTS').hwApi : 'https://api.jdsharecode.xyz/api/'}ddfactory/${randomCount}`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
